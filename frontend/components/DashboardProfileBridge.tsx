...
const menuCopyStyle = {
  minWidth: 0, width: '100%', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gridTemplateRows: 'auto auto',
  alignContent: 'center', justifyItems: 'start', gap: 3, overflow: 'hidden',
};
...
export default function DashboardProfileBridge() {
  ...
  useEffect(() => {
    ...
    const applyModuleVisibility = (access: ModuleAccess[], role: Role) => {
      ...
      const selectors = [
        ...(!documentsAccess ? [
          '.app .nav-item[href="/documents"]', '.app .nav-item[href="/reminders"]', '.app .nav-item[href^="/documents?"]',
          '.app .kpi[href="/documents"]', '.app .kpi[href^="/documents?"]', '.app .documents-panel', '.app .recent-docs',
          '.app .heading-actions a[href="/documents/new"]', '.app .quick-grid a[href^="/documents"]',
        ] : []),
        ...(!officeAccess ? [
          '.app .nav-item[href^="/office"]', '.app .kpi[href^="/office"]', '.app .operations', '.app .integration',
          '.app .heading-actions a[href^="/office"]', '.app .quick-grid a[href^="/office"]',
        ] : []),
      ];
      ...
      document.querySelectorAll('.app .nav-item, .app .kpi, .app .documents-panel, .app .recent-docs, .app .operations, .app .integration, .app .heading-actions a, .app .quick-grid a').forEach((element) => {
        const selector = [...hidden].find((value) => {
    ...
    const loadAccess = async (currentToken: string | null) => {
      ...
      try {
        ...
        setUser({ name: payload.name ?? readUser().name, email: payload.email, role: payload.role ?? readUser().role });
        applyModuleVisibility(payload.moduleAccess ?? [], payload.role ?? 'VIEWER');
      } catch {
        applyModuleVisibility([], 'VIEWER');
      }
    };
  ...
  const renderMenuItem = (href: string, icon: string, title: string, description: string) => (
    <Link href={href} role="menuitem" onClick={() => setOpen(false)} style={itemStyle}>
      ...
      <span style={menuCopyStyle}><strong style={menuTitleStyle}>{title}</strong><small style={menuDescriptionStyle}>{description}</small></span>
    </Link>
  );

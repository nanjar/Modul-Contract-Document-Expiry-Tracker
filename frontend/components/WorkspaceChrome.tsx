...
const copy = {
  en: {
    ...
    dark: 'Dark', light: 'Light', signOut: 'Sign out', profile: 'Profile', account: 'Account', security: 'Security',
  },
  id: {
    ...
    dark: 'Gelap', light: 'Terang', signOut: 'Keluar', profile: 'Profil', account: 'Akun', security: 'Keamanan',
  },
} as const;
...
function sessionUserFromToken(): SessionUser {
  try {
    ...
    return { name: payload.name ?? payload.email ?? 'User', email: payload.email, role: (payload.role ?? 'VIEWER') as Role };
...
export default function WorkspaceChrome({ children }: Props) {
  ...
  const [userName, setUserName] = useState('User'); const [query, setQuery] = useState(''); const [archiveActive, setArchiveActive] = useState(false); const [profileOpen, setProfileOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const syncAuth = () => { const next = sessionUserFromToken(); setRole(next.role); setUserName(next.name); };
    window.addEventListener('expiry-tracker-auth-change', syncAuth); return () => window.removeEventListener('expiry-tracker-auth-change', syncAuth);
  }, []);

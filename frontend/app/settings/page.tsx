...
export default function SettingsPage() {
  ...
  async function load() {
    ...
    if (response.status === 401 || response.status === 403) {
      setError('You do not have permission to manage system settings.');
      return;
    }
    if (!response.ok) throw new Error('Unable to load settings.');
    const data = await response.json();
    setSettings(data);
    setThreshold(String(data.warningThresholdDays));
  }
  ...
  async function save(e: FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/settings`, {
        ...
        body: JSON.stringify({ warningThresholdDays: Number(threshold) }),
      });
      const data = await response.json();
  ...
  const card: React.CSSProperties = { background: '#fff', border: '1px solid #e8ecf2', borderRadius: 18, padding: 24, marginBottom: 16, boxShadow: '0 12px 30px rgba(25,40,70,.05)' };

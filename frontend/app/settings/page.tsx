...
export default function SettingsPage() {
  ...
  const permissionCard: CSSProperties = { ...card, borderColor: '#efb7b7', color: '#a33a3a', background: '#fffafa' };
  ...
  return <main style={{ minHeight: '100vh', background: '#f6f8fc', padding: '32px' }}><div style={{ maxWidth: 1000, margin: '0 auto' }}>
    ...
    {authorized !== true ? <div role="alert" style={permissionCard}>You do not have permission to manage system settings.</div> : <>
      {error && <div role="alert" style={{ ...card, borderColor: '#efb7b7', color: '#a33a3a' }}>{error}</div>}
      <section style={card}><h2 style={{ margin: '0 0 6px', fontSize: 18, color: '#17213a' }}>Expiry status</h2><p style={{ color: '#718096', fontSize: 14 }}>Documents at or before this number of calendar days from today are shown as <strong>Expiring Soon</strong>. This is a system-wide setting.</p><form noValidate onSubmit={save} style={{ display: 'flex', alignItems: 'end', gap: 12, flexWrap: 'wrap', marginTop: 18 }}><label style={{ display: 'grid', gap: 7, color: '#34415c', fontSize: 13, fontWeight: 700 }}>Warning threshold (days)<input aria-label="Warning threshold in days" type="number" min={0} max={3650} step={1} value={threshold} onChange={(e) => setThreshold(e.target.value)} style={{ width: 190, border: '1px solid #dce2eb', borderRadius: 10, padding: '11px 12px', fontSize: 15 }} /></label><button disabled={saving || !settings} style={{ border: 0, borderRadius: 10, padding: '12px 18px', background: '#4c6fff', color: '#fff', fontWeight: 800 }}>{saving ? 'Saving…' : 'Save settings'}</button></form>{message && <div role="status" style={{ marginTop: 12, color: '#257a4b', fontSize: 13 }}>{message}</div>}</section>

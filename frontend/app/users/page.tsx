...
type User = {
  id: string; email: string; name: string; role: Role; isActive: boolean; createdAt: string; updatedAt: string;
};
...
export default function UsersPage() {
  ...
  function reset() { setEditing(null); setName(''); setEmail(''); setPassword(''); setRole('VIEWER'); setFormError(''); setOpen(false); }
  function edit(user: User) { setEditing(user); setName(user.name); setEmail(user.email); setPassword(''); setRole(user.role); setFormError(''); setOpen(true); }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const response = await fetch(editing ? `${API_URL}/users/${editing.id}` : `${API_URL}/users`, {
        ...
        headers: (() => { const headers = getHeaders(); headers.set('Content-Type', 'application/json'); return headers; })(),
  ...
  return <main style={{ minHeight: '100vh', background: '#f6f8fc', padding: '32px' }}>
    <div style={{ maxWidth: 1180, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 20, marginBottom: 28 }}>
        <div><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '.12em', textTransform: 'uppercase', color: '#70809b' }}>Administration</div><h1 style={{ margin: '7px 0', fontSize: 32, color: '#17213a' }}>Users & access</h1><p style={{ margin: 0, color: '#718096' }}>Manage users, roles and account status from one secure workspace.</p></div>
        {authorized && <button onClick={() => { reset(); setOpen(true); }} style={primaryButtonStyle}>+ Add user</button>}
      </div>
      ...
      {!authorized ? <div role="alert" style={permissionStyle}>You do not have permission to manage users and access.</div> : <>
        ...
        {open && <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.42)', display: 'grid', placeItems: 'center', padding: 20, zIndex: 50 }}><form noValidate onSubmit={save} style={{ width: '100%', maxWidth: 500, background: '#fff', borderRadius: 20, padding: 28, boxShadow: '0 25px 80px rgba(0,0,0,.2)' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div><h2 style={{ margin: 0, color: '#17213a' }}>{editing ? 'Edit user' : 'Add user'}</h2><p style={{ margin: '6px 0 22px', color: '#718096', fontSize: 13 }}>{editing ? 'Update identity and access role.' : 'Create an account with the appropriate access level.'}</p></div><button type="button" onClick={reset} style={{ border: 0, background: 'transparent', fontSize: 22, color: '#8792a4' }}>×</button></div>{formError && <div role="alert" style={formErrorStyle}>{formError}</div>}<label style={fieldLabelStyle}>Name<input value={name} onChange={(event) => { setName(event.target.value); setFormError(''); }} type="text" style={fieldInputStyle} /></label><label style={fieldLabelStyle}>Email{editing ? <input value={email} type="email" disabled aria-describedby="immutable-email-note" style={{ ...fieldInputStyle, background: '#f3f5f8', color: '#69758a', cursor: 'not-allowed' }} /> : <input value={email} onChange={(event) => { setEmail(event.target.value); setFormError(''); }} type="email" style={fieldInputStyle} />} {editing && <small id="immutable-email-note" style={{ display: 'block', marginTop: 6, fontWeight: 500, color: '#8a96a8' }}>Email cannot be changed after the account is created.</small>}</label><label style={fieldLabelStyle}>Password<input value={password} onChange={(event) => { setPassword(event.target.value); setFormError(''); }} type="password" placeholder={editing ? 'Leave blank to keep current password' : ''} style={fieldInputStyle} /></label><label style={fieldLabelStyle}>Role<select value={role} onChange={(event) => { setRole(event.target.value as Role); setFormError(''); }} style={{ ...fieldInputStyle, background: '#fff' }}><option value="VIEWER">Viewer</option><option value="EDITOR">Editor</option><option value="SUPERUSER">Superuser</option></select></label><div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 25 }}><button type="button" onClick={reset} style={secondaryButtonStyle}>Cancel</button><button type="submit" disabled={saving} style={primaryButtonStyle}>{saving ? 'Saving…' : 'Save user'}</button></div></form></div>}
      </>}
    </div>
  </main>;
}

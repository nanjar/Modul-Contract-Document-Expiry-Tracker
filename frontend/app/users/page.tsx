...
export default function UsersPage() {
  ...
  async function save(event: FormEvent) {
    ...
    setSaving(true);
    try {
      const body: { name: string; email: string; role: Role; password?: string } = { name: name.trim(), email: email.trim(), role };
      if (password) body.password = password;
      const response = await fetch(editing ? `${API_URL}/users/${editing.id}` : `${API_URL}/users`, { method: editing ? 'PATCH' : 'POST', headers: (() => { const headers = getHeaders(); headers.set('Content-Type', 'application/json'); return headers; })(), body: JSON.stringify(editing ? body : { ...body, password }) });
      if (response.status === 401 || response.status === 403) { setAuthorized(false); return; }

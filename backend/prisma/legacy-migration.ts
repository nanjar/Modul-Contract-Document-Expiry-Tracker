...
type Result = { sheet: string; read: number; imported: number; skipped: number; invalid: number; duplicates: number; needsReview: number; errors: string[] };
...
const result = (sheet: string, read: number): Result => ({ sheet, read, imported: 0, skipped: 0, invalid: 0, duplicates: 0, needsReview: 0, errors: [] });
...
const requestStatus = (v: unknown): OfficeRequestStatus | undefined => { const s = String(v ?? '').trim().toUpperCase(); return (Object.values(OfficeRequestStatus) as string[]).includes(s) ? s as OfficeRequestStatus : undefined; };
...
async function main() {
  ...
  for (const row of employees) {
    ...
    if (!email || !name) { er.invalid++; er.errors.push('Missing Email or Name'); continue; }
    try {
      if (apply) {
  ...
  for (const row of attendance) {
    ...
    if (action !== 'checkin' && action !== 'checkout') { ar.needsReview++; ar.errors.push(`Unsupported attendance action '${action || '-'}'`); continue; }
    const userId = text(row.ChatId) ? chats.get(text(row.ChatId)!) : undefined, day = dt(row.Date);
    if (!userId || !day || userId.startsWith('DRY_RUN:')) { ar.invalid++; ar.errors.push(`Unresolved employee/date ChatId=${text(row.ChatId) ?? '-'}`); continue; }
  ...
  for (const sheet of ['LeaveRequests','LateRequests','WFHRequests','OvertimeRequests','ReimbursementRequests','BusinessTripRequests','AssetRequests']) {
    ...
    for (let i = 0; i < sourceRows.length; i++) {
      ...
      if (!requesterId || requesterId.startsWith('DRY_RUN:')) { rr.invalid++; rr.errors.push(`Unresolved employee ChatId=${text(row.ChatId) ?? '-'}`); continue; }
      const status = requestStatus(row.Status);
      if (!status) { rr.invalid++; rr.errors.push(`Invalid status '${text(row.Status) ?? '-'}' at row ${i + 2}`); continue; }
  ...
  for (let i = 0; i < announcementRows.length; i++) {
    ...
    if (!title || !message) { nr.invalid++; nr.errors.push(`Announcement ${key} missing Title/Content`); continue; }
    ...
    try {
      ...
      if (!creatorId) { nr.invalid++; nr.errors.push(`Announcement ${key} has no resolvable creator`); continue; }
  ...
  for (const row of readRows) {
    ...
    if (!announcementId || !userId || userId.startsWith('DRY_RUN:')) { rd.invalid++; rd.errors.push(`Unresolved announcement/employee: ${text(row.AnnouncementID) ?? '-'} / ${text(row.ChatId) ?? '-'}`); continue; }
  ...
  for (let i = 0; i < meetings.length; i++) {
    ...
    if (!creatorId || creatorId.startsWith('DRY_RUN:') || !start || !end) { mr.invalid++; mr.errors.push(`Invalid meeting row ${i + 2}`); continue; }
    ...
    try {
      ...
      const booking = await prisma.meetingBooking.create({ data: { roomId: room.id, createdById: creatorId, title, startsAt: start, endsAt: end, status: statusFromLegacy(row.Status), metadata: row as object } });
      ...
      mr.imported++;
    } catch (e) { mr.skipped++; mr.errors.push(`${i + 2}: ${String(e)}`); }
  }
  results.push(mr);

  for (const sheet of ['Sessions','Dashboard']) { const count = (sheets[sheet] ?? []).length; results.push({ sheet, read: count, imported: 0, skipped: count, invalid: 0, duplicates: 0, needsReview: count, errors: [`${sheet} is derived/operational legacy data and is intentionally not persisted as a source table.`] }); }

  mkdirSync(reportDir, { recursive: true });
  const report = { generatedAt: new Date().toISOString(), source, mode: apply ? 'APPLY' : 'DRY_RUN', results, totals: results.reduce((a, r) => ({ read: a.read + r.read, imported: a.imported + r.imported, skipped: a.skipped + r.skipped, invalid: a.invalid + r.invalid, duplicates: a.duplicates + r.duplicates, needsReview: a.needsReview + r.needsReview }), { read: 0, imported: 0, skipped: 0, invalid: 0, duplicates: 0, needsReview: 0 }) };
...
function statusFromLegacy(value: unknown) {
  const normalized = String(value ?? '').trim().toUpperCase();
  if (normalized === 'CONFIRMED') return OfficeRequestStatus.APPROVED;
  if (normalized === 'CANCELLED') return OfficeRequestStatus.CANCELLED;
  if (normalized === 'REJECTED') return OfficeRequestStatus.REJECTED;
  if (normalized === 'PENDING') return OfficeRequestStatus.PENDING;
  return OfficeRequestStatus.PENDING;
}

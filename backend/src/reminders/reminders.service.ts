...
export class RemindersService {
  ...
  async create(documentId: string, input: { daysBefore: number; enabled: boolean }, actorId: string) {
    ...
    const existing = await this.prisma.reminder.findUnique({
      where: { documentId_daysBefore: { documentId, daysBefore: input.daysBefore } },
    });

    if (existing) {
      throw new ConflictException('Reminder already exists for this interval');
    }
    ...
    await this.audit.log({
      ...
      metadata: { documentId, daysBefore: reminder.daysBefore, enabled: reminder.enabled },
    });
  ...
  async update(reminderId: string, input: { daysBefore?: number; enabled?: boolean }, actorId: string) {
    ...
    if (input.daysBefore !== undefined && input.daysBefore !== existing.daysBefore) {
      const duplicate = await this.prisma.reminder.findUnique({
        where: {
    ...
    await this.audit.log({
      ...
      entityId: reminder.id,
      metadata: {
        previous: { daysBefore: existing.daysBefore, enabled: existing.enabled },
        current: { daysBefore: reminder.daysBefore, enabled: reminder.enabled },
      },
    });
  ...
  async remove(reminderId: string, actorId: string) {
    ...
    await this.audit.log({
      ...
      entity: 'Reminder',
      entityId: reminderId,
      metadata: { documentId: existing.documentId, daysBefore: existing.daysBefore },
    });
    ...
    return this.prisma.$transaction(
      DEFAULT_REMINDER_DAYS.map((daysBefore) =>
        this.prisma.reminder.upsert({
          ...
          update: {},
          create: { documentId, daysBefore, enabled: true },
        }),
      ),
    );
  }
  ...
  private async assertDocument(documentId: string) {
    const document = await this.prisma.document.findUnique({
      where: { id: documentId },
      select: { id: true, archivedAt: true, expiryDate: true, reminderEnabled: true },
    });

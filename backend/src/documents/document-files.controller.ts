...
export class DocumentFilesController {
  ...
  async upload(@Param('id') id: string, @UploadedFile() file: Express.Multer.File, @Req() req: { user: { sub: string } }) {
    ...
    if (file.size > Number(process.env.MAX_FILE_SIZE_BYTES ?? 10485760)) throw new BadRequestException('File exceeds the maximum allowed size');
    ...
    return this.storage.upload(id, req.user.sub, { buffer: file.buffer, originalname: file.originalname, mimetype: file.mimetype, size: file.size });
  }

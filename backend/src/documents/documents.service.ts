...
export class DocumentsService {
  ...
  async list(query?: { search?: string; status?: string; documentType?: string; expiryFrom?: string; expiryTo?: string; sort?: string; page?: number; limit?: number }) {
    ...
    return { items: documents.map((document) => this.toResponse(document, warningDays)), pagination: { page, limit, total, totalPages: Math.ceil(total / limit) } };
  }

  async findOne(id: string) {
    const document = await this.prisma.document.findUnique({ where: { id } });
    if (!document || document.archivedAt) throw new NotFoundException('Document not found');
    return this.toResponse(document, await this.settings.getWarningThresholdDays());
  }
  ...
  async create(input: any) {
    ...
    const document = await this.prisma.document.create({ data: {
      ...
      ownerId: input.ownerId || null, issueDate: input.issueDate ? new Date(input.issueDate) : null,
  ...
  async update(id: string, input: any, actorId: string) {
    ...
    if (expiryChanged || reminderSettingChanged) await this.reminders.resetDeliveryState(document.id, document.reminderEnabled);
    if (document.expiryDate && document.reminderEnabled && (expiryChanged || !existing.expiryDate || !existing.reminderEnabled)) await this.reminders.createDefaults(document.id);
    return this.toResponse(document, await this.settings.getWarningThresholdDays());
  }

  async attachFile(id: string, file: { storageKey: string; originalFilename: string; mimeType: string; fileSize: number }, actorId: string) {
    const existing = await this.prisma.document.findUnique({ where: { id } });
    if (!existing || existing.archivedAt) throw new NotFoundException('Document not found');
  ...
  private async assertOwnerExists(ownerId: string) {
    const owner = await this.prisma.user.findUnique({ where: { id: ownerId }, select: { id: true, isActive: true } });
    if (!owner || !owner.isActive) throw new BadRequestException('Document owner is not an active user');
  }

  private toResponse(document: any, warningDays: number) {
    return { ...document, fileSize: typeof document.fileSize === 'bigint' ? document.fileSize.toString() : document.fileSize, status: calculateConfigurableDocumentStatus(document.expiryDate, document.archivedAt, new Date(), warningDays) };
  }
}

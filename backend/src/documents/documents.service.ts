...
export class DocumentsService {
  ...
  async list(query?: {
    ...
    if (status === DocumentStatus.EXPIRED) {
      where.expiryDate = { ...(where.expiryDate ?? {}), lt: today };
    }
    ...
    if (status === DocumentStatus.ACTIVE) {
      where.expiryDate = { ...(where.expiryDate ?? {}), gt: threshold };
    }
  ...
  async update(id: string, input: any, actorId: string) {
    ...
    const expiryChanged =
      ...
      (document.expiryDate?.getTime() ?? null);
    const reminderSettingChanged =
      existing.reminderEnabled !== document.reminderEnabled;
    ...
    if (
      ...
      (expiryChanged || !existing.expiryDate || !existing.reminderEnabled)
    ) {
      await this.reminders.createDefaults(document.id);
    }
  ...
  private getOrderBy(sort?: string) {
    switch (sort) {
      ...
      case 'expiry_desc':
        return [
          { expiryDate: { sort: 'desc' as const, nulls: 'last' as const } },
          { createdAt: 'desc' as const },
        ];
      case 'title_asc':
  ...
  private status(document: { expiryDate: Date | null; archivedAt: Date | null }) {
    return calculateDocumentStatus(document.expiryDate, document.archivedAt);
  }
}

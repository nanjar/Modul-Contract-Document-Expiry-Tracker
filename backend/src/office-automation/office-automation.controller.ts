...
export class OfficeAutomationController {
  constructor(
    ...
    private readonly requestRules: OfficeRequestRulesService,
  ) {}

  @Get('dashboard')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_DASHBOARD_VIEW')
  dashboard(@Req() req: any) { return this.office.dashboard(req.user.sub); }

  @Get('reports')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_REPORT_VIEW')
  reports(@Req() req: any) { return this.queries.report(req.user.sub); }

  @Get('users')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_TASK_ASSIGN')
  users(@Req() req: any) { return this.queries.usersForOffice(req.user.sub); }
  ...
  list(@Req() req: any, @Query('all') all?: string) {
    ...
    return this.office.list(isPrivileged && all === 'true' ? undefined : req.user.sub, req.user.sub);
  }

  @Get('requests/:id')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_REQUEST_VIEW')
  findOne(@Param('id') id: string, @Req() req: any) { return this.office.findOne(id, req.user.sub); }

  @Post('requests')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_REQUEST_CREATE')
  create(@Body() dto: CreateOfficeRequestDto, @Req() req: any) {
    this.requestRules.validate(dto.type, dto.metadata);
    return this.office.create(dto, req.user.sub);
  }

  @Patch('requests/:id')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_REQUEST_EDIT')
  update(@Param('id') id: string, @Body() dto: UpdateOfficeRequestDto, @Req() req: any) { return this.office.update(id, dto, req.user.sub); }

  @Post('requests/:id/cancel')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_REQUEST_EDIT')
  cancel(@Param('id') id: string, @Req() req: any) { return this.office.cancel(id, req.user.sub); }
  ...
  @Get('tasks/:id')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_TASK_VIEW')
  task(@Param('id') id: string, @Req() req: any) { return this.queries.task(id, req.user.sub); }

  @Get('approvals')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_APPROVAL_VIEW')
  listApprovals(@Req() req: any, @Query('all') all?: string) { return this.queries.approvals(req.user.sub, all === 'true'); }

  @Post('requests/:id/tasks')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_TASK_ASSIGN')
  createTask(@Param('id') id: string, @Body() dto: CreateOfficeTaskDto, @Req() req: any) { return this.office.createTask(id, dto, req.user.sub); }

  @Patch('tasks/:id')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_TASK_UPDATE')
  updateTask(@Param('id') id: string, @Body() dto: UpdateOfficeTaskDto, @Req() req: any) { return this.office.updateTask(id, dto, req.user.sub); }

  @Post('requests/:id/approvals')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_APPROVAL_ACTION')
  createApproval(@Param('id') id: string, @Body() dto: CreateOfficeApprovalDto, @Req() req: any) { return this.office.createApproval(id, dto, req.user.sub); }

  @Post('approvals/:id/decision')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_APPROVAL_ACTION')
  decideApproval(@Param('id') id: string, @Body() dto: DecideOfficeApprovalDto, @Req() req: any) { return this.office.decideApproval(id, dto, req.user.sub); }

  @Post('approvals/:id/approve')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_APPROVAL_ACTION')
  approve(@Param('id') id: string, @Req() req: any) { return this.office.decideApproval(id, { status: 'APPROVED' }, req.user.sub); }

  @Post('approvals/:id/reject')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_APPROVAL_ACTION')
  reject(@Param('id') id: string, @Body() body: { comment?: string }, @Req() req: any) { return this.office.decideApproval(id, { status: 'REJECTED', comment: body?.comment }, req.user.sub); }

  @Get('requests/:id/attachments')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_REQUEST_VIEW')
  listAttachments(@Param('id') id: string, @Req() req: any) { return this.attachments.list(id, req.user.sub); }
  ...
  @UseInterceptors(FileInterceptor('file', {
    ...
    fileFilter: (_req, file, callback) => {
      if (!ALLOWED_MIME_TYPES.has(file.mimetype)) { callback(new BadRequestException(`Unsupported file type: ${file.mimetype}`), false); return; }
      callback(null, true);
    },
  }))
  ...
  @Get('attachments/:id')
  @ModuleAccess(ModuleKey.OFFICE_AUTOMATION, 'OFFICE_REQUEST_VIEW')
  downloadAttachment(@Param('id') id: string, @Req() req: any) { return this.attachments.download(id, req.user.sub); }
}
